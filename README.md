...
> - **100% Offline & Zero Telemetry:** PostHog tracking libraries and network telemetry have been completely purged.
> - **Low-Resource & Laptop Tuned:** Includes OpenBLAS CPU acceleration profiles and Cargo thread bounding (`CARGO_BUILD_JOBS=2`) to run smoothly on machines with 8 GB RAM and integrated graphics.
> - **Cleaned Architecture:** Purged committed binaries (`vs_buildtools.exe`), obsolete Electron configurations, and legacy unmaintained Python backend archives.
